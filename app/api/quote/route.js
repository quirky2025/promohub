...
export async function POST(req) {
  try {
    ...
    if (deliveryAddress && deliveryAddress.trim() && link.company_id) {
      try {
        const { data: co } = await supabase.from('companies').select('billing_address').eq('id', link.company_id).single();
        const ba = co?.billing_address;
        const empty = !ba || (typeof ba === 'object' && Object.keys(ba).length === 0);
        if (empty) await supabase.from('companies').update({ billing_address: { full: deliveryAddress.trim() } }).eq('id', link.company_id);
      } catch {}
    }

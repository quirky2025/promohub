-- Outdoor & Sports manual review list
-- READ ONLY. Run this if outdoor_sports_normalization_summary_READONLY.sql shows manual_review_products > 0.
...
select
  concat_ws(' | ', current_category || ' / ' || coalesce(current_subcategory, '(blank)'), supplier_sku, name) as manual_review_item
from suggested

    ...
    const emailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 620px; margin: 0 auto; color: #1a1a1a;">
        ...
        <div style="background: #fff; border: 1px solid #E0DDD7; border-top: none; padding: 28px 32px; border-radius: 0 0 12px 12px;">
          ...
          ${customer.requiredDate ? `<h2 style="font-size: 14px; color: #1B2A4A; margin: 20px 0 8px; text-transform: uppercase; letter-spacing: 0.08em;">Required Date</h2><p style="font-size: 14px; color: #3D3A36; margin: 0 0 16px;">${(() => { try { return new Date(customer.requiredDate + 'T00:00:00').toLocaleDateString('en-AU'); } catch { return customer.requiredDate; } })()}</p>` : ''}
          ${customer.purpose ? `<h2 style="font-size: 14px; color: #1B2A4A; margin: 20px 0 8px; text-transform: uppercase; letter-spacing: 0.08em;">Purpose / Event</h2><p style="font-size: 14px; color: #3D3A36; margin: 0 0 16px;">${customer.purpose}</p>` : ''}

...
const DECORATION_FAMILY_ORDER = [
  ...
  ['Spot Colour Print', ['screen print','pad print','offset','puff']],
...
export function materialFamiliesOf(materialTags) {
  const map = { cotton: 'Cotton', metal: 'Metal', plastic: 'Plastic' };
  const out = new Set();
  for (const t of materialTags || []) { const f = map[String(t).toLowerCase()]; if (f) out.add(f); }
  return [...out];
}

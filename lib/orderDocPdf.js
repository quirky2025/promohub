...
export async function generateOrderDocPDF(o) {
  ...
  right(docTitle, width - 40, height - 35, 18, fB, WHITE);
  // Invoice # (INV…) is distinct from Order # (OC…). If no explicit invoiceNumber
  // is passed, derive it from the order number by swapping the OC prefix for INV.
  const invNo = o.invoiceNumber || String(o.orderNumber || '').replace(/^OC/i, 'INV');
  ...
  page.drawRectangle({ x: 40, y: y - 84, width: 245, height: 78, borderColor: LINEC, borderWidth: 0.8 });
  page.drawRectangle({ x: 300, y: y - 84, width: 255, height: 78, borderColor: LINEC, borderWidth: 0.8 });
  const bill = [o.customer.company, o.customer.name, o.customer.email, o.customer.phone].filter(Boolean);
  let by = y - 16;
  bill.forEach((t, i) => { page.drawText(String(t), { x: 48, y: by, size: 9, font: i === 0 ? fB : fR, color: BLACK }); by -= 12; });
  // Deliver To: company, recipient, phone, then the FULL address (wrapped).
  const rawDelv = String(o.deliveryAddress || 'TO BE CONFIRMED');
  const delvParts = (rawDelv.includes('\n') ? rawDelv.split('\n') : rawDelv.split(',')).map(s => s.trim()).filter(Boolean);
  const toks = s => new Set(String(s).toLowerCase().replace(/[^a-z0-9 ]/g, ' ').split(/\s+/).filter(w => w.length > 1));
  const dedup = [];
  delvParts.forEach(p => {
    if (dedup.length) {
      const a = toks(p), b = toks(dedup[dedup.length - 1]);
      let common = 0; a.forEach(w => { if (b.has(w)) common++; });
      if (a.size && b.size && common / Math.min(a.size, b.size) >= 0.6) return;
    }
    dedup.push(p);
  });
  const addrLines = [];
  ...
  dedup.forEach(p => {
    if (!dcur) dcur = p;
    else if ((dcur + ', ' + p).length <= 40) dcur += ', ' + p;
    else { addrLines.push(dcur); dcur = p; }
  });
  if (dcur) addrLines.push(dcur);
  const deliverLines = [];
  if (o.customer.company) deliverLines.push({ t: o.customer.company, bold: true });
  if (o.customer.name) deliverLines.push({ t: 'Attn: ' + o.customer.name });
  if (o.customer.phone) deliverLines.push({ t: String(o.customer.phone) });
  addrLines.forEach(t => deliverLines.push({ t }));
  let dy = y - 16;
  deliverLines.slice(0, 6).forEach(line => { page.drawText(String(line.t).substring(0, 44), { x: 312, y: dy, size: 9, font: line.bold ? fB : fR, color: BLACK }); dy -= 12; });
  ...
  page.drawText(`Production lead time is ${o.leadTimeDays || '3-7'} business days (varies by branding method).`, { x: 40, y: y - 39, size: 8.5, font: fR, color: BLACK });

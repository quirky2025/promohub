...
## 🟠 NOTE（总控 2026-06-18）：admin 图片加载 —— 配合 Cloudinary 止血

> 背景:Cloudinary 用量 6/19 暴涨(bot + 原始 JPG)。已做:robots 挡 bot、Strict Transformations + referral 白名单(`quirkypromo.com.au`)、PDF/ZIP off、静态图迁 `/public`。admin 这边要配合,**别再当烧额度的大户**。

**`admin-list-pagination` 分支要点:**
1. **admin 产品列表分页**(如 50/页)—— 别一次加载几千个产品(=几千张图)。这是 admin 当前最烧 Cloudinary 的点。
2. 每个产品**只加载一张小缩略图(首图)**,**不要在 list/grid/card 上加载 gallery 全部图**。
3. 缩略图走 **`ProductImg` size="thumb"** 或固定 `f_auto,q_auto,w_160`,**lazy loading**;别裸写 `<img src={原始cloudinaryUrl}>`。
4. ⚠️ Cloudinary 已开 **Strict Transformations**:线上域名(quirkypromo.com.au)下生成缩略图 OK,但 **Vercel preview(*.vercel.app)上新尺寸会被挡 → 图裂是预期,不是 bug**(要 preview 出图就把 vercel 域名加进 referral)。
5. 静态站点图(logo/hero/category)已迁 `/public`,**admin 别再引用 Cloudinary 那几张**。

完成后把 preview/diff 带回总控按清单验收。

**✅ 完成(2026-06-20):** artworks 页 3 处图改 `ProductImg`(thumb + lazy + 占位);产品列表本就分页 50/页且无产品图;静态图无 Cloudinary 引用。diff 极小(`app/admin/artworks/page.js`)。⚠️ **preview 图裂为预期**(referral 白名单未含 `*.vercel.app`);生产正常。分支 `admin-list-pagination` patch 已交,Lily 自行推。

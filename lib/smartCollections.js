...
// A product's material FAMILIES — identical derivation to the front-end
// Material facet (lib/filterConfig.js materialBase), so collection rules and
// the sidebar filter always agree (Aluminium / Stainless Steel etc. are
// derived from name + materials text, not just raw material_tags).
...
async function fetchRuleHits(db, r, limit) {
  ...
  if (r.material_tags.length) {
    // Material rule matches by FAMILY (same as the sidebar filter), not raw tags.
    const wanted = new Set(r.material_tags.map(m => String(m).toLowerCase()));
    hits = hits.filter(p => productMaterialFamilies(p).some(f => wanted.has(String(f).toLowerCase())));
  }

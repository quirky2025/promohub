...
import {
  ...
  capacityBucketBags, materialFamiliesOf, stockTypeOf,
...
// facet = { key, label, isType?, get:(p)=>string[] }
const colour   = { key: 'colour',   label: 'Colour',          get: p => colourFamiliesOf(p.colour_slugs) };
const price    = { key: 'price',    label: 'Price',           get: p => arr(priceBucket(p._price)) };
const moq      = { key: 'moq',      label: 'Min Qty',         get: p => arr(moqBucket(p.min_qty)) };
...
const stock    = { key: 'stock',    label: 'Stock / Lead time', get: p => arr(stockTypeOf(p.fulfillment)) };

export const GLOBAL = [colour, price, moq, brand, branding, stock];

const bagType   = { key: 'bagType',   label: 'Bag Type',   isType: true, get: p => arr(p.subcategory) };
const capacity  = { key: 'capacity',  label: 'Capacity',   get: p => arr(capacityBucketBags(p.capacity)) };
const material  = { key: 'material',  label: 'Material',   get: p => materialFamiliesOf(p.material_tags) };

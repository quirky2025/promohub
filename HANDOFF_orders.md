...
## 待改(Lily 测试中陆续记)
- **Get a Quote 弹窗易误关**:点遮罩就关闭 + 清空已填内容,容易误触。修:点遮罩不关(只 ✕ 关),或已填内容时关闭前确认。组件:components/QuoteModal.jsx(或 QuoteButton/QuoteBuilder)。
- **后台 Orders 列表 "Reply →"**:是 mailto 链接,没默认邮件程序就无反应。可改成"复制客户邮箱"或详情内写回复。(Lily 待定要不要改)

## 🔍 一致性审计(2026-06-24)— "同一逻辑两处各写一份"的漂移问题
触发:同一产品同一印刷,客户端报价 $7,259 vs 后台 $7,260(取整时机不同)。已修(ProductClient 先取整单价再×qty,对齐 QuoteBuilder + checkout)。
扫描发现还需统一:
- **交期 lead time 不一致(客户可见,优先)**:产品页 "3-7 business days"(ProductClient 750/1261/1315)、下单确认 "5-7"(order-confirmation:59)、报价 PDF "7"(quote route leadTimeDays 默认7)、supply-chain/logistics "5-7"。**待 Lily 拍板统一成几天**(建议 "5-7 business days from artwork approval")。
- **价格常量散落 12+ 文件**(现值都一样,但易漂移):MARGIN(1.40)、GST(0.10)、SHIPPING(30)、SETUP_FEE(40) —— 见 ProductClient/QuoteBuilder/cart/place-order/各分类页。ProductJsonLd 用 1.4(同值)。
- **branding 文案 brandingLabel** 两处各写(ProductClient + QuoteBuilder)—— 已都修 laser,但仍是复制粘贴,易再漂移。
**根治方案**:建 `lib/quoteConfig.js`(或 lib/pricing.js)集中 MARGIN/GST/SHIPPING/SETUP_FEE/LEAD_TIME + 共享 brandingLabel/算价函数,全站 import,物理上不可能再漂移。动 ~12 文件,需谨慎(FUSE 写盘不稳,逐个 Read 核验)。

## 交期 lead time 决定(2026-06-24)
- **现在全站统一 "3-7 business days"**(标准,按印刷方式略不同)。全站 = 产品页/Order Placed/OC/Invoice/Quote PDF/各邮件/QuoteBuilder 字段(默认3-7,文本可填)。
- 这是 **Trends 产品的标准**;以后接别的供应商,lead time 按供应商不同 → 归入供应商配置(和成本/运费/setup 一起)。
- 产品页运输时效表(2–5/5–7/5–15 air/sea)是运输,保留不动。

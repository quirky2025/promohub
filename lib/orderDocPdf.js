...
export async function generateOrderDocPDF(o) {
  ...
  right(docTitle, width - 40, height - 44, isInvoice ? 22 : 18, fB, WHITE);
  const meta = [
    ['Order #', o.orderNumber],
    ['Order Date:', o.date],
    ['Page:', '1 of 1'],
  ...
  const payH = !isInvoice ? 50 : (isPaid ? 80 : 150);
  const notesH = 48;
  const FOOTER_TOP = 72;
  ...
  if (isInvoice) {
    ...
    if (isPaid) {
      ...
      page.drawText('This tax invoice is for your records.', { x: 52, y: y - 30, size: 8.5, font: fR, color: GREY });
      y -= 58;
    } else {
      const bk = o.bank || {};
    ...
    page.drawText('A separate tax invoice is attached with payment details.', { x: 52, y: y - 30, size: 8.5, font: fR, color: GREY });
    y -= 50;
  }
  ...
  page.drawText(`Production lead time is ${o.leadTimeDays || 7} business days from artwork approval and full payment.`, { x: 40, y: y - 26, size: 8.5, font: fR, color: GREY });
  page.drawText('We start production once you approve your free digital proof.', { x: 40, y: y - 39, size: 8.5, font: fR, color: GREY });

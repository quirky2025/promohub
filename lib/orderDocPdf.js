...
export async function generateOrderDocPDF(o) {
  ...
  const docTitle = isPaid ? 'TAX INVOICE' : 'ORDER CONFIRMATION';
  ...
  cb.forEach(([l, v]) => {
    ...
    page.drawText(v, { x: 40 + fB.widthOfTextAtSize(l, 9) + 5, y: cy, size: 9, font: fR, color: rgb(0.85, 0.85, 0.87) });
    cy -= 12;
  });
  right(docTitle, width - 40, height - 44, isPaid ? 22 : 18, fB, WHITE);
  const meta = [
    [isPaid ? 'Invoice #' : 'Order #', o.orderNumber],
    [isPaid ? 'Invoice Date:' : 'Order Date:', o.date],
    ['Page:', '1 of 1'],
  ];
  let my = height - 62;
  meta.forEach(([l, v]) => {
    page.drawText(l, { x: 392, y: my, size: 9, font: fB, color: WHITE });
    right(String(v || ''), width - 40, my, 9, fR, rgb(0.85, 0.85, 0.87));
    my -= 12;
  });
  ...
  const drawRow = (it) => {
    ...
    page.drawRectangle({ x: 40, y: y - rowH + 14, width: width - 80, height: rowH, color: LIGHT });
    ...
    let ddy = y - 14;
    det.forEach(d => { page.drawText(d.substring(0, 70), { x: 120, y: ddy, size: 7.5, font: fR, color: GREY }); ddy -= 11; });
  ...
  const totRow = (label, value, opt = {}) => {
    ...
    page.drawText(label, { x: 358, y, size: sz, font: opt.big ? fB : fR, color: opt.big ? BLACK : GREY });
  ...
  // ── PAYMENT BOX ──
  if (isPaid) {
    page.drawRectangle({ x: 40, y: y - 40, width: width - 80, height: 46, color: LIGHT, borderColor: GREEN, borderWidth: 0.6 });
    page.drawText('Payment received — thank you!', { x: 52, y: y - 14, size: 10, font: fB, color: GREEN });
    page.drawText('Paid by credit card. A tax invoice for your records is attached.', { x: 52, y: y - 30, size: 8.5, font: fR, color: GREY });
    y -= 58;
  } else {
    const bk = o.bank || {};
    page.drawRectangle({ x: 40, y: y - 110, width: width - 80, height: 116, color: LIGHT, borderColor: GOLD, borderWidth: 0.6 });
    ...
    rows.forEach(([l, v], i) => {
      const ry = y - 34 - i * 13;
      page.drawText(l, { x: 52, y: ry, size: 8.5, font: fR, color: GREY });
      page.drawText(String(v), { x: 175, y: ry, size: 8.5, font: l === 'Reference' ? fB : fR, color: l === 'Reference' ? GOLD : BLACK });
    });
    page.drawText('Prefer to pay by credit card? Call us on 02 9477 4748 — a 2% surcharge applies.', { x: 52, y: y - 102, size: 8, font: fR, color: GREY });
    y -= 128;
  }

  // ── NOTES ──
  page.drawRectangle({ x: 40, y: y - 50, width: width - 80, height: 56, color: CREAM });
  page.drawText(`NOTE: Estimated production lead time is ${o.leadTimeDays || 7} business days from artwork approval and receipt of full payment.`, { x: 52, y: y - 18, size: 8.5, font: fB, color: BLACK });
  page.drawText('NOTE: Production begins only after you approve your free digital proof.', { x: 52, y: y - 38, size: 8.5, font: fB, color: BLACK });

  // ── FOOTER ──
  page.drawLine({ start: { x: 40, y: 50 }, end: { x: width - 40, y: 50 }, thickness: 0.5, color: LINEC });
  page.drawText('Thank you for your order. We look forward to creating something great for you.', { x: 40, y: 36, size: 7.5, font: fR, color: GREY });
  page.drawText('Quirky Promo  ·  ABN 95 656 714 270  ·  quirkypromo.com.au  ·  hello@quirkypromo.com.au', { x: 40, y: 24, size: 7.5, font: fR, color: GREY });

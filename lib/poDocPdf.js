...
export async function generatePurchaseOrderPDF(o) {
  ...
  // ── DELIVER TO (customer) ──
  let y = H - 138;
  page.drawText('DELIVER TO', { x: M, y, size: 8, font: fB, color: GREY });
  y -= 15;
  const d = o.deliver || {};
  if (d.name) { page.drawText(d.name, { x: M, y, size: 12, font: fB, color: NAVY }); y -= 14; }
  if (d.phone) { page.drawText('Phone: ' + d.phone, { x: M, y, size: 10, font: fR, color: BLACK }); y -= 13; }
  (d.address || '').split('\n').filter(Boolean).forEach(line => { page.drawText(line, { x: M, y, size: 10, font: fR, color: BLACK }); y -= 13; });

  // ── JOB (top-right of body) ──
  if (o.jobName) {
    page.drawText('JOB', { x: 310, y: H - 138, size: 8, font: fB, color: GREY });
    page.drawText(String(o.jobName), { x: 310, y: H - 153, size: 11, font: fB, color: NAVY });
  }
  ...
  page.drawLine({ start: { x: lblX, y: y + 6 }, end: { x: cTot, y: y + 6 }, thickness: 1, color: NAVY });

...
export function cld(url, width) {
  ...
  // Migrated TRENDS images now live on R2 — never serve them from Cloudinary
  // (the account may be deactivated). toR2 returns null for non-TRENDS URLs.
  const r2 = toR2(url, width);
  if (r2) return r2;

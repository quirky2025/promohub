...
export async function generatePurchaseOrderPDF(o) {
  ...
  // WinAnsi-safe: pdf-lib's standard fonts can't encode non-Latin characters (e.g. Chinese).
  // On any encoding failure, strip to ASCII so a stray character never crashes the whole PO PDF.
  const _ascii = (t) => String(t ?? '').replace(/[^\x00-\x7F]/g, '');
  const _drawText = page.drawText.bind(page);
  page.drawText = (t, opts) => { try { return _drawText(String(t ?? ''), opts); } catch { return _drawText(_ascii(t), opts); } };
  const tw = (t, s, f) => { try { return f.widthOfTextAtSize(String(t ?? ''), s); } catch { return f.widthOfTextAtSize(_ascii(t), s); } };

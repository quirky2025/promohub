-- Gear For Life transform preview check.
...
checks as (
  ...
  union all select 'excluded_rows', (select count(*) from excluded_rows)::int, 1, coalesce((select jsonb_agg(jsonb_build_object('source_row_number', source_row_number, 'supplier_sku', supplier_sku, 'raw_name', raw_name)) from excluded_rows), '[]'::jsonb)
  union all select 'transform_preview_rows', (select count(*) from preview_rows)::int, 471, '{}'::jsonb
  union all select 'preview_ready_rows', (select count(*) from preview_rows where mapping_status = 'ready')::int, 428, '{}'::jsonb
  union all select 'preview_needs_review_rows', (select count(*) from preview_rows where mapping_status = 'needs_review')::int, 8, coalesce((select jsonb_agg(jsonb_build_object('supplier_sku', supplier_sku, 'raw_name', raw_name, 'target_category', target_category, 'target_subcategory', target_subcategory, 'review_notes', review_notes) order by supplier_sku) from preview_rows where mapping_status = 'needs_review'), '[]'::jsonb)
  union all select 'preview_blocked_rows', (select count(*) from preview_rows where mapping_status = 'blocked')::int, 35, coalesce((select jsonb_agg(jsonb_build_object('supplier_sku', supplier_sku, 'raw_name', raw_name, 'warning_flags', warning_flags) order by supplier_sku) from preview_rows where mapping_status = 'blocked'), '[]'::jsonb)
  union all select 'manual_confirmed_category_rows', (select count(*) from preview_rows where mapping_rule_id = 'gfl_manual_confirmed_category')::int, 9, '{}'::jsonb
  union all select 'keyword_cleanup_ready_rows', (select count(*) from preview_rows where mapping_status = 'ready' and mapping_rule_id like 'product_keyword_cleanup:%')::int, 37, '{}'::jsonb
  union all select 'keyword_cleanup_needs_review_rows', (select count(*) from preview_rows where mapping_status = 'needs_review' and mapping_rule_id like 'product_keyword_cleanup:%')::int, 8, '{}'::jsonb
  union all select 'category_mapping_rows', (select count(*) from preview_rows where mapping_rule_id like 'category_mapping_v4:%')::int, 382, '{}'::jsonb
  union all select 'odgp_normalized_rows', (select count(*) from preview_rows where supplier_sku = 'ODGP(P)' and preview_json ->> 'raw_supplier_sku' = 'ODGP')::int, 1, '{}'::jsonb

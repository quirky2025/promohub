-- Gear For Life products INSERT - DRAFT.
...
sku_conflicts as (
  ...
  elsif preview_count <> 471 then
    raise exception 'Expected 471 GFL transform preview rows. Stop.';
  elsif ready_count <> 436 then
    raise exception 'Expected 436 ready rows. Stop.';
  elsif blocked_count <> 35 then
    raise exception 'Expected 35 blocked discontinued rows. Stop.';
  elsif needs_review_count <> 0 then
  ...
  elsif ready_price_count <> 773 then
    raise exception 'Expected 773 ready product price tiers. Stop.';
  end if;
end $$;
...
sku_conflicts as (
  ...
  where prod.supplier = 'Gear For Life';

  if inserted_products <> 436 then
    raise exception 'Inserted GFL product count mismatch: expected %, got %.', 436, inserted_products;
  end if;

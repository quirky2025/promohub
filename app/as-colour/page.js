...
// not in the global TRENDS category pages). Type-first: grouped by category > subcategory.
...
// Category display order in the Range.
const CAT_ORDER = ['Apparel', 'Bags', 'Headwear'];

async function getRangeProducts(supplier) {
  const { data, error } = await supabase
    .from('products')
    .select('id, slug, name, category, subcategory, brand, pricing_tiers(base_price), product_colours(images, sort_order)')
...
// Group products: category -> subcategory -> [products]
  ...
  return (
    <Link href={`/products/${p.slug}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
      <div style={{ border: '1px solid #E0DDD7', borderRadius: '12px', overflow: 'hidden', background: '#fff' }}>
        <div style={{ aspectRatio: '1', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff', overflow: 'hidden' }}>
          {img
        ...
        {cats.map((cat) => (
          <div key={cat} style={{ marginBottom: '20px' }}>
            {Object.keys(groups[cat]).sort().map((sub) => (
              <div key={sub} style={{ marginBottom: '36px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '18px' }}>
                  {groups[cat][sub].map((p) => <Card key={p.id} p={p} />)}
                </div>
              </div>
            ))}
          </div>

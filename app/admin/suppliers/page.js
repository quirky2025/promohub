...
export default function AdminSuppliersPage() {
  ...
  async function save() {
    ...
    setAdding(false);
    setForm({ name: '', contact_name: '', email: '', phone: '', payment_terms: 'prepaid', notes: '' });
    load();
  }
    ...
    <div style={{ fontFamily: '"DM Sans", sans-serif', background: '#fff', minHeight: '100vh' }}>
        ...
        <nav style={{ display: 'flex', gap: '2px' }}>
          {NAV.map(n => (
            <Link key={n.href} href={n.href}
              style={{ color: n.href === '/admin/suppliers' ? '#fff' : 'rgba(255,255,255,0.7)', textDecoration: 'none', padding: '6px 12px', borderRadius: '6px', fontSize: '13px', fontWeight: n.href === '/admin/suppliers' ? 700 : 500, background: n.href === '/admin/suppliers' ? 'rgba(255,255,255,0.1)' : 'none' }}>
              {n.label}
            </Link>
          ))}
        </nav>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '28px 32px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
          ...
          <button onClick={() => setAdding(true)} style={{ background: GOLD, color: '#fff', border: 'none', borderRadius: '8px', padding: '8px 16px', fontSize: '13px', fontWeight: 700, cursor: 'pointer' }}>+ Add supplier</button>
        </div>
      ...
      {adding && (
        <div onClick={e => e.target === e.currentTarget && setAdding(false)}
          ...
          <div style={{ background: '#fff', borderRadius: '16px', padding: '28px', maxWidth: '420px', width: '100%' }}>
            <h2 style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: '22px', color: NAVY, margin: '0 0 16px' }}>Add supplier</h2>

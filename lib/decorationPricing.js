...
// 丝印版费表(每位置/每套网)。8 色照 AS 价目 = $400(非 8×$60)。
const SCREEN_SETUP = { 1: 60, 2: 120, 3: 180, 4: 240, 5: 300, 6: 360, 7: 420, 8: 400 };
...
export function quoteDecoration({ method, colours = 1, positions = 1, qty = MIN_QTY, shade = 'white', sizeKey, dark = false, poly = false } = {}) {
  ...
  if (method === 'screen_print') {
    ...
    setup = (SCREEN_SETUP[nCol] || (m.setup || 60) * nCol) * pos; // 版费按表(8色=$400),×位置
  } else if (method === 'dtg') {
...
// Decoration type from a product's category/subcategory. MUST stay identical to
// decoType() in app/products/[slug]/page.js + ASColourClient (used for the same
// "from $X" pricing). Used by Range cards so their price == the PDP "From".
export function productDecoType(product) {
...
// Card "From $X" for a calculator product (garment base = min pricing_tiers.base_price).
// Returns the decorated starting unit price (== PDP "From"), or null.
export function calculatorFromPrice(product) {
  const bases = (product?.pricing_tiers || [])
    .map((t) => Number(t.base_price))
    .filter((n) => Number.isFinite(n) && n > 0);
  const garmentBase = bases.length ? Math.min(...bases) : 0;
  return startingUnitPrice(garmentBase, productDecoType(product));
}
...
// ── 收尾 / 履约 add-on(D 组)。per_unit 为保本成本;客户价 = ×MARGIN。
// fold_bag 抓绒件用 per_unit_fleece。E/F(re-labelling / neck tag)复杂 → 前端走询价,不在此。
export const FINISHING = [
  { key: 'swing_tag', label: 'Swing tag', per_unit: 0.50 },
  { key: 'apply_barcode', label: 'Apply barcode', per_unit: 0.50 },
  { key: 'fold_bag', label: 'Fold & bag', per_unit: 1.50 },
  { key: 'individual_bags', label: 'Individually bagged', per_unit: 0.30 },
];

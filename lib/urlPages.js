...
// up to N representative product images pulled from each category's products. SEO-safe (live only).
export async function getCategoryHubCards(maxChildren = 4, imgPerCard = 3) {
  const { data, error } = await supabase
    ...
    .order('priority', { ascending: true })
    .order('nav_label', { ascending: true });
  if (error) { console.error('[url_pages] getCategoryHubCards failed', error); return []; }
  const rows = data || [];
  const tops = rows.filter((p) => p.page_type === 'product_category' && !p.breadcrumb_parent && !p.noindex);
  return Promise.all(tops.map(async (top) => {
    const children = rows
    ...
    if (catName) {
      const { data: prods } = await supabase
        .from('products').select('product_colours(images, sort_order)')
        .eq('is_published', true).eq('category', catName).limit(12);

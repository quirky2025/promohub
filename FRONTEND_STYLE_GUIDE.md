...
| 正文字色（浅底上） | `#000` | **纯黑。绝不用灰字**（`#1a1a1a` / `#5A5A5A` / `#7A7570` / `#888` / `#A9A39B` 等都禁止做正文；连近黑 `#1a1a1a` 也算灰，一律用纯黑 `#000`） |
...
**铁律：** 浅色背景 = 纯白 `#fff`；浅色背景上的字 = 纯黑 `#000`（不是 `#1a1a1a`，那也算灰）。深色区域 = navy `#1B2A4A` + 白字。除此之外别造新灰。
...
- **裸价 margin 按档位分级**（`lib/pricing.js` 的 `tierMargin(index)`，index 按数量从小到大、0=第一档）：
  - 第 1 档（最小量）→ **×1.50（50%）**
  - 第 2 档 → **×1.45（45%）**
  - 第 3 档及以后 → **×1.40（40%）**
  - 只作用于**裸价 / garment**。所有"按档显示/计算裸价"的地方都必须用 `tierMargin(i)`，不要写死 `× MARGIN`。
- **印刷 decoration per-unit** → 仍是 `cost × MARGIN(1.40)` 再**向上取整到 $0.10**（`decoUnitPrice()`），**不分档**。
- **Setup** → 统一 flat **$60**（不 ×margin）。
- **quote_only（indent 询价，如 LN / Intex 大货）** → 参考价 "From $X" 用 **MARGIN(1.40)**,**不分档**。
- **卡片 "From $X"** → 取最便宜的档（=最大量=第3档+=40%），所以卡片价**不受分档影响**。
- 购物车存的是算好的 `unitPrice`/`grand`,下单**不重算**——页面价=实际收款。
...
_最后更新：2026-07-11（新增 §7.1 分档 margin）。改动全站风格时同步更新这份文件。_

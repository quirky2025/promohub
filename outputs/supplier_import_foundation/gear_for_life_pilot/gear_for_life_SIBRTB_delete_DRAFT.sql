-- Gear For Life: DELETE discontinued product SIBRTB -- DRAFT, WRITE OPERATION.
-- Run gear_for_life_SIBRTB_delete_preview_READONLY.sql FIRST and confirm the counts.
--
-- SAFETY:
--   * Scoped strictly to supplier = 'Gear For Life' AND supplier_sku = 'SIBRTB'.
...
with target as (
  ...
  where supplier = 'Gear For Life' and supplier_sku = 'SIBRTB'
)
...
from public.products where supplier='Gear For Life' and supplier_sku='SIBRTB'
union all select 'remaining_images', count(*)::int
from public.product_images pi
  join public.products p on p.id=pi.product_id
where p.supplier='Gear For Life' and p.supplier_sku='SIBRTB'

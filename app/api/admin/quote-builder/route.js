...
export async function GET(request) {
  ...
  try {
    ...
    if (custQ) {
      ...
      const [comps, cts, adr] = await Promise.all([
        ...
        db.from('company_addresses').select('*').in('company_id', ids).limit(200),
      ]);
      const compMap = {}; (comps.data || []).forEach(c => { compMap[c.id] = c; });
      const adrByCo = {}; (adr && !adr.error ? adr.data : []).forEach(a => { (adrByCo[a.company_id] = adrByCo[a.company_id] || []).push(a); });
      const deliveryFor = (cid) => fmtAddrRow(pickDefaultDelivery(adrByCo[cid])) || fmtBilling(compMap[cid]?.billing_address);
      const customers = (cts.data || []).map(c => { const co = compMap[c.company_id] || {}; return { company_id: c.company_id, company: co.name || '', delivery: deliveryFor(c.company_id), name: [c.first_name, c.last_name].filter(Boolean).join(' '), email: c.email || '', phone: c.phone || '' }; });
      (comps.data || []).forEach(co => { if (!customers.some(x => x.company_id === co.id)) customers.push({ company_id: co.id, company: co.name || '', delivery: deliveryFor(co.id), name: '', email: '', phone: '' }); });

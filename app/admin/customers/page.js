...
export default function AdminCustomersPage() {
  ...
  function open(c) { setSelected(c); setAddrDraft(c.delivery_address || c.address || ''); setBillDraft(c.billing_address_str || ''); setIndDraft(c.industry || ''); }
  ...
  async function patch(action, payload) {
    ...
    const data = await res.json();
    setSaving(false);
    if (data.ok && payload.kind) {
      const upd = payload.kind === 'billing' ? { billing_address_str: data.address } : { delivery_address: data.address, address: data.address, has_address: !!data.address };
      setCustomers(prev => prev.map(x => x.id === selected.id ? { ...x, ...upd } : x));
      setSelected(prev => ({ ...prev, ...upd }));
    } else if (data.customer) {
      setCustomers(prev => prev.map(x => x.id === selected.id ? { ...x, ...data.customer } : x));
      setSelected(prev => ({ ...prev, ...data.customer }));
    }
  }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: selected ? '1.5fr 1fr' : '1fr', gap: '16px', alignItems: 'start' }}>
          ...
          {selected && (
            <div style={{ background: '#fff', border: '1px solid #E0DDD7', borderRadius: '12px', padding: '20px', position: 'sticky', top: '16px' }}>
              ...
              <div style={{ fontSize: '11px', color: '#7A7570', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '6px' }}>Delivery address <span style={{ color: '#9B958E', fontWeight: 400, textTransform: 'none' }}>(on quotes / shipping)</span></div>
              <textarea value={addrDraft} onChange={e => setAddrDraft(e.target.value)} rows={2} placeholder="Street, suburb, state, postcode" style={{ width: '100%', padding: '9px 12px', border: '1px solid #E0DDD7', borderRadius: '8px', fontSize: '13px', resize: 'vertical', boxSizing: 'border-box', background: '#fff' }} />
              <button onClick={() => patch('address', { address: addrDraft, kind: 'delivery' })} disabled={saving} style={{ background: GOLD, color: '#fff', border: 'none', borderRadius: '8px', padding: '7px 14px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', marginTop: '8px', marginBottom: '14px' }}>{saving ? 'Saving…' : 'Save delivery'}</button>

              <div style={{ fontSize: '11px', color: '#7A7570', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '6px' }}>Billing address <span style={{ color: '#9B958E', fontWeight: 400, textTransform: 'none' }}>(on invoices)</span></div>
              <textarea value={billDraft} onChange={e => setBillDraft(e.target.value)} rows={2} placeholder="Leave blank = same as delivery" style={{ width: '100%', padding: '9px 12px', border: '1px solid #E0DDD7', borderRadius: '8px', fontSize: '13px', resize: 'vertical', boxSizing: 'border-box', background: '#fff' }} />
              <button onClick={() => patch('address', { address: billDraft, kind: 'billing' })} disabled={saving} style={{ background: '#fff', color: NAVY, border: '1px solid ' + NAVY, borderRadius: '8px', padding: '7px 14px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', marginTop: '8px', marginBottom: '16px' }}>{saving ? 'Saving…' : 'Save billing'}</button>

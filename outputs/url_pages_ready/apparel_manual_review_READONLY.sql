-- Apparel manual review detail
-- READ ONLY. Run after apparel_normalization_summary_READONLY.sql when manual review rows remain.
-- Lists the exact products that the Apparel rules intentionally did not auto-map.
...
select
  ...
  is_published,
  current_tags,
  case when has_workwear_signal then 'workwear' end as would_add_workwear,
  case when has_teamwear_signal then 'teamwear' end as would_add_teamwear
from suggested

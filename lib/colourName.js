...
// Display formatter: some suppliers ship colour names ALL CAPS ("RED", "GUN METAL").
// SHOUTY names → Title Case for display; mixed-case names pass through untouched
// (so curated names like "Navy Blue" are never mangled). Display-only — data,
// slugs and variant matching all keep the raw value.
export function formatColourName(name) {
  const s = String(name || '').trim();
  if (!s) return s;
  if (s !== s.toUpperCase()) return s; // already mixed case → leave alone
...
export function cleanColour(raw) {
  ...
  if (low === 'custom' || /any colou?r|produced in|full colou?r|design (your|my) own|your own design/i.test(c)) {

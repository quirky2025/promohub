...
export async function POST(request) {
  try {
    ...
    // Internal email to QuirkyPromo team
    const internalHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 620px; margin: 0 auto; color: #1a1a1a;">
        ...
        <div style="background: #fff; border: 1px solid #E0DDD7; border-top: none; padding: 28px 32px; border-radius: 0 0 12px 12px;">
          ...
          ${notes ? `
          <h2 style="font-size: 13px; color: #1B2A4A; margin: 0 0 12px; padding-bottom: 8px; border-bottom: 1px solid #F0EEED; text-transform: uppercase; letter-spacing: 0.08em;">💬 Additional Notes</h2>
          <div style="background: #F8F7F4; border-radius: 8px; padding: 16px; font-size: 14px; color: #3D3A36; white-space: pre-wrap; margin-bottom: 24px;">${notes}</div>
          ` : ''}
    ...
    const customerHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 620px; margin: 0 auto; color: #1a1a1a;">
        ...
        <div style="background: #fff; border: 1px solid #E0DDD7; border-top: none; padding: 28px 32px; border-radius: 0 0 12px 12px;">
          ...
          <p style="font-size: 15px; margin: 0 0 24px;">Thank you for your sourcing enquiry! We've received your request and our team will review your requirements and get back to you within <strong>24–48 business hours</strong> with a detailed quotation.</p>
          ...
          <div style="background: #1B2A4A; border-radius: 10px; padding: 20px 24px; margin: 0 0 24px;">
            <div style="font-size: 13px; color: rgba(255,255,255,0.6); margin-bottom: 12px; text-transform: uppercase; letter-spacing: 0.05em;">What Happens Next</div>
            ${['Our sourcing team reviews your requirements', 'We identify the best factory and product match', 'You receive a detailed quote with pricing and lead times', 'We discuss any questions and refine the brief'].map((s, i) => `
              <div style="display: flex; gap: 12px; margin-bottom: 10px; align-items: flex-start;">
                <div style="width: 22px; height: 22px; border-radius: 50%; background: #C9A96E; color: #1B2A4A; font-size: 11px; font-weight: 700; display: flex; align-items: center; justify-content: center; flex-shrink: 0; text-align: center; line-height: 22px;">${i + 1}</div>
                <div style="font-size: 13px; color: rgba(255,255,255,0.8); padding-top: 3px;">${s}</div>
              </div>
            `).join('')}
          </div>

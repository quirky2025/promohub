...
const TYPES = [
  { key: 'factory_po',       label: 'Supplier PO' },
  { key: 'factory_invoice',  label: 'Supplier invoice' },
...
export default function OrderDocuments({ orderNumber }) {
  ...
  return (
    <div style={{ marginTop: 8 }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: '#000', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 10 }}>📎 Documents / Evidence</div>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', background: '#FDFBF7', border: '1.5px dashed #D8CFC0', borderRadius: 10, padding: 10, marginBottom: 12 }}>
        ...
        {busy && <span style={{ fontSize: 12, color: NAVY, fontWeight: 700 }}>Uploading…</span>}
        <span style={{ fontSize: 11, color: '#000' }}>Pick a type, then choose a file (image / PDF)</span>
      </div>

      {/* list */}
      {loading ? (
        <div style={{ fontSize: 12, color: '#000' }}>Loading…</div>
      ) : docs.length === 0 ? (
        <div style={{ fontSize: 12, color: '#000' }}>No documents yet. Upload PO, supplier invoice, payment proof, product images, etc.</div>
      ) : (
            ...
            <div key={d.id} style={{ width: 150, border: '1.5px solid #E0DDD7', borderRadius: 10, overflow: 'hidden', background: '#fff' }}>
              ...
              <div style={{ padding: '7px 9px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 5 }}>
                  <a href={d.file_url} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: NAVY, fontWeight: 700 }}>Open →</a>
                  <button onClick={() => del(d.id)} style={{ background: 'none', border: 'none', color: '#991B1B', fontSize: 11, cursor: 'pointer', padding: 0 }}>Delete</button>
                </div>
              </div>
            </div>
          ))}

...
// ---------------- Pens: BARREL material only ----------------
// "<material> <component>" collocations (stainless steel BALL/TIP, metal CLIP,
// silicone GRIP…) are stripped BEFORE the primary scan, so a plastic pen whose
// text opens with "stainless steel ball" is still classed Plastic (the barrel).
// 'ball(?!\s?point)' avoids eating "ballpoint"; 'barrel' itself is never stripped.
const PEN_COMPONENT_COLLOCATION = /\b(?:stainless\s+steel|recycled\s+alumini\w+|alumini\w+|aluminum|metal|chrome|brass|steel|silicone|rubber|plastic|abs|bamboo|wood(?:en)?)\s+(?:ball(?!\s?point)|tip|nib|clip|grip|ring|band|trim|accents?|plunger|button|end|top)s?\b/gi;
export function penPrimaryMaterial(materials, name, allow) {
  const clean = (s) => String(s || '').replace(PEN_COMPONENT_COLLOCATION, ' ');
  return primaryBottleMaterial(clean(materials), clean(name), allow);
}

...
export const THIN_HIDE = 2; // §6: hide a facet value with fewer than this many products
...
export function computeFacets(products, category, { includeType = true, thin = THIN_HIDE } = {}) {
  ...
  return facets.map(f => {
    ...
    const ord = f.order; // ordered facets (price/moq/capacity) sort by this, not by count
    const minCount = f.noThin ? 1 : thin; // stock/eco: always show (don't hide count-1 lead times)
    const values = [...counts.entries()]
      .map(([value, count]) => ({ value, count }))
      .filter(x => x.count >= minCount)
      .sort((a, b) => ord
        ? ord.indexOf(a.value) - ord.indexOf(b.value)

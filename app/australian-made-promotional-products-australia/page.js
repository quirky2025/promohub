  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const { data } = await supabase
        .from('products')
        .select('id, name, slug, supplier_sku, category, subcategory, is_australian_made, min_qty, is_published, quote_only, product_colours(images, sort_order), pricing_tiers(base_price)')
    ...
    <div style={{ fontFamily: '"DM Sans", sans-serif', minHeight: '100vh', background: '#ffffff', color: '#000' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #E0DDD7', padding: '12px 40px' }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto', fontSize: '13px', color: '#000' }}>
          ...
          <span style={{ color: NAVY, fontWeight: 600 }}>Australian Made</span>
        </div>
      </div>
      ...
      <div style={{ background: NAVY, padding: '56px 40px', textAlign: 'center' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          <div style={{ display: 'inline-block', background: '#2D6A4F', color: '#fff', fontSize: '11px', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', padding: '5px 14px', borderRadius: '20px', marginBottom: '18px' }}>🇦🇺 Made in Australia</div>
          ...
          <p style={{ fontSize: '16px', color: 'rgba(255,255,255,.75)', lineHeight: 1.7, margin: 0 }}>
            Branded merchandise manufactured and decorated right here in Australia — support local, cut lead times, and back your brand with genuine Aussie-made quality.
          </p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '48px 40px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '28px', flexWrap: 'wrap', gap: '16px' }}>
          <h2 style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: '30px', color: NAVY, margin: 0, fontWeight: 600 }}>
            Australian Made Products {!loading && <span style={{ fontSize: '15px', color: '#000', fontWeight: 400 }}>({filtered.length})</span>}
          </h2>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '80px 0', color: '#000' }}>Loading Australian Made products…</div>
        ) : filtered.length === 0 ? (
                  ...
                  <Link key={product.id} href={`/products/${product.slug}`} style={{ textDecoration: 'none' }}>
                    ...
                    >
                      <div style={{ height: '190px', background: '#ffffff', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative' }}>
                        {img
                          ? <img src={cld(img, 400)} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'contain', padding: '12px' }} />
                          : <div style={{ fontSize: '40px', color: '#D0CCC8' }}>📦</div>}
                        {product.is_australian_made && (
                          <div style={{ position: 'absolute', top: '10px', left: '10px', background: '#2D6A4F', color: '#fff', fontSize: '10px', fontWeight: 700, padding: '3px 8px', borderRadius: '20px' }}>🇦🇺 AUS MADE</div>
                        )}
                      </div>

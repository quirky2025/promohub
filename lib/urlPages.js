...
export async function getProductsForUrlPage(urlPage, limit = 96) {
  ...
  let query = supabase
    ...
    // AS/Gildan calculator products live only in their brand Range (AS Colour /
    // Gildan), not in global category/collection pages — different pricing model.
    // Keep NULLs (standard products) and anything that isn't 'calculator'.
    .or('decoration_model.is.null,decoration_model.neq.calculator')
    .order('name')
    .limit(limit);

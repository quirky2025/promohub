...
**库存同步(trends-stock)的行为,免得下次又被搞糊涂:**
- 每天自动跑一次(`vercel.json` 里配置,悉尼凌晨4点),同步的是**全部** Trends 产品(现在 2108 个),不是只挑新导入的那几个——这是它的本职工作,数据会越用越旧,全库存都要定期刷新。
- Trends 这边库存要一个个 SKU 去调用官方接口查(不像 PB 是翻目录页一次性批量扫),所以单次执行时间根本不够处理完 2108 个,一次触发只能推进一部分,要么等第二天自动续跑,要么人工重复点同一个网址(safe to repeat)。**这从功能一开始建起来就是这样,不是哪次改坏的。**
- PB(PromoBrands)不一样:PB 是翻供应商目录页一次性批量匹配,自己库里的 PB 产品全部匹配到就提前收工,所以之前那次能"一次触发、16秒搞定"——这不是 Trends 的行为标准,两边机制本来就不同,不要拿 PB 的"一次搞定"去预期 Trends。
- "自动分片"指的是**一次请求内部**拆成一小段一小段处理(有时间预算,用完就得交卷),不是"这次点了、以后不用管、它自己接力跑到全部完成"——跨请求的自动接力目前没做,想要的话可以加(让它交卷前自己再拍一次请求给自己)。
- 上次(D11 首次上线)看起来像是"一次全量搞定",其实也是反复触发了很多次,只是我在后台帮她点完、点到跑完了才汇报结果,她没看到中间过程。这次她自己在点、看到了所有中间结果,才觉得"怎么变麻烦了"——机制没变,只是这次她亲眼看到了过程。
- 如果只是想赶紧把"新导入这一批"的库存补上,不需要陪着全库存一起等,应该给 trends-stock 加一个 `sku=` 之类的指定名单参数(参照 backfill-pb-colours 的做法),只同步这一小撮,几秒钟搞定,不用跟着 2000 多个不相关的一起排队。
...
**2026-07-24 补充 · 31 个 SKU "supplier 明明是 Trends 但码不是 Trends 格式" 真相大白:**
- 完整调查见 `outputs/ISSUE-trends-vs-pb-sku-mismatch.md`,结论:**这批行创建时就是 `importPB()` 正确建的(created_at 时间线、图片 R2 路径、SKU 尾随空格三条证据互相印证),`supplier` 字段是建行之后被一条 7/23 当天在 Supabase SQL Editor 里手动跑、但没存档的 UPDATE 语句改成 'Trends' 的——不是导入器本身的 bug,导入器所有历史版本都是硬编码对的供应商。**
- **新家规:凡是在 Supabase SQL Editor 里跑的写语句(UPDATE/DELETE/INSERT),跑之前必须先把语句存进 `outputs/` 里存档,不能只在 SQL Editor 窗口里跑完就关掉。** 这次就是因为当天那条 UPDATE 没存档,查了一整个 session 才靠图片路径/时间线这种间接证据破案,教训很大。
- 待做(防再发):`supplier` 字段加变更审计触发器;`(supplier, upper(trim(supplier_sku)))` 唯一约束。

**2026-07-24 补充 · PB 颜色系统三个连环坑(同一批产品第一次真正被人工挑出来):**
1. **"Bottle"这种假颜色标签没有真的清干净**:`dropNonColourInventoryTags()` 过滤规则代码早就写了(见上方 07-23 记录),但只对**以后**的导入生效,S898.06 这种已经建库的行从没真正跑过这条回填,导致代码修了、数据没修,活生生留在线上被 Lily 抓到。**教训:凡是"改了导入器代码"的修复,必须同时确认存量数据有没有真正跑过回填,不能假设写了代码 = 数据已经对了。**
2. **颜色选项(`products.colours`)和库存表(`product_stock`)是两条不同流水线,会互相对不上**:`products.colours` 由导入/回填时的 `displayColourName()`/`dropNonColourInventoryTags()` 清洗过;`product_stock.colour_name` 由每天库存同步(`pbColour()`)直接用供应商原始文字,完全没清洗。两边独立跑,时间一长/数据一乱就会出现"色块叫 Blue,库存表却叫 Black"这种对不上(S406 实例)。**这一批先手工用 SQL 挨个改的名字对齐,根治要让两边共用同一套颜色命名清洗规则,还没做,记入待办。**
3. **色块图片"找不到真图就随便拿一张凑数"的前端 bug**:`app/products/[slug]/page.js` 里的 `colourImages[i]` 位置图池兜底,对供应商自动导入的产品(`fromColoursField` 为真,颜色本来就该有明确匹配结果)来说,"没匹配上"不该拿图池瞎配一张不相关的宣传/客户使用照——S777 实例:好几个颜色配到了跟颜色完全无关的品牌宣传图。**已改:这类产品找不到真图直接留空显示色块,图池兜底只保留给老的手工建品体系。**
- 待办:把这套颜色清洗规则贯通到全部 61 个 PB D15 产品(目前只手工修了 S898.06/S406/S777 三个被截图抓到的,其余可能还有没被发现的同类问题)。

**2026-07-24 补充 · 库存同步接口会无限期卡死,不只是"变慢":**
- `?only=pb` 手动补跑接口之前发现过一次卡住 10+ 分钟不返回,查出根因是接口内部两处地方都没有超时保护:① 调 PB 官方接口的 `fetch()`(对方一卡住,我们的函数就跟着无限期挂起);② 查我们自己数据库那段循环用的 Supabase 查询本身也没有超时。**已加:PB 接口调用加 15 秒超时;数据库查询加 `withTimeout()` 包装;外层再加一个 270 秒总看门狗,不管卡在哪一步,到点强制返回诊断信息,不再让人对着卡死的请求干等。**
- 教训:任何调用外部 API 或数据库的 `await`,只要没有超时保护,对方一卡住我们就会跟着无限期挂起,表现出来就是"越来越慢"甚至彻底不返回,不是真的"数据量大所以慢"——两种情况长得很像,遇到"怎么这么慢"先怀疑有没有漏加超时,再怀疑是不是真的数据量大。
